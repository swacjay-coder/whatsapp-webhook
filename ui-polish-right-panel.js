...
next = next.replace(
  /const BOT_VERSION = "[^"]*";/,
  'const BOT_VERSION = "iconic-team-inbox-v31-5-8-60-3-1-11-unified-functional-sidebar";'
);
...
const functionalNavHtml = `      <nav class="sidebar-nav" aria-label="Team Inbox navigation">
...
next = next.replace(/      <nav class="sidebar-nav" aria-label="Team Inbox navigation">[\s\S]*?      <\/nav>/, functionalNavHtml);
...
if (!next.includes(cssMarker)) {
  ...
  if (styleEndIndex !== -1) next = next.slice(0, styleEndIndex) + css + "\n  " + next.slice(styleEndIndex);
}
...
`;
if (!next.includes(sidebarJsMarker)) next = next.replace("\nloadMessages();", sidebarJs + "\nloadMessages();");

if (next !== source) {
  fs.writeFileSync(serverPath, next, "utf8");
  console.log("Unified functional sidebar polish applied.");
} else {
  console.log("Unified functional sidebar polish already applied.");
}
